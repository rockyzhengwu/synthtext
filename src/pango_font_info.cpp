...
//
//std::string fontconfig_tmpdir="/tmp";
//
//
//std::string fonts_dir="/home/zhengwu/CLionProjects/synthtext/han_fonts";
...
void PangoFontInfo::SoftInitFontConfig() {
  if (fonts_dir_.empty()) {
    HardInitFontConfig(fonts_dir_.c_str(),
                       cache_dir_.c_str());
  }
}
...
void PangoFontInfo::HardInitFontConfig(const std::string& fonts_dir,
  ...
  std::string fonts_conf_file = cache_dir_ +  "/fonts.conf";
  // todo 配置写到配置文件中
  ...
  if (FcInitReinitialize() != FcTrue) {
    std::cout<< "FcInitiReinitialize failed!!\n";
  }
  FontUtils::ReInit();
...
bool PangoFontInfo::ParseFontDescription(const PangoFontDescription *desc) {
  ...
  if (!family) {
    ...
    std::cout<< "WARNING: Could not parse family name from description: '%s'\n"  << desc_str << std::endl;
    g_free(desc_str);
    return false;
  }
...
bool PangoFontInfo::CoversUTF8Text(const char* utf8_text) const {
  ...
  icu::UnicodeString ucs = icu::UnicodeString::fromUTF8(icu::StringPiece(utf8_text));
...
int PangoFontInfo::DropUncoveredChars(std::string& utf8_text) const {
  ...
  std::string new_str = "";
  PangoCoverage* coverage = pango_font_get_coverage(font, nullptr);
  icu::UnicodeString ucs = icu::UnicodeString::fromUTF8(icu::StringPiece(utf8_text.c_str()));

  for (int it=0 ; it < ucs.length(); ++it) {
    int unicode = ucs[it];
    std::string  word;
    ...
    if(new_str.length()==0 && IsWhitespace(unicode)){
      continue;
    }
...
//  const UNICHAR::const_iterator it_end = UNICHAR::end(utf8_char.c_str(), utf8_char.length());
  icu::UnicodeString ucs = stringToUnicodeString(utf8_char);
  for (int it=0; it<ucs.length();  ++it) {
    int unicode = ucs[it];
    PangoGlyph glyph_index = pango_fc_font_get_glyph(reinterpret_cast<PangoFcFont*>(font), unicode);
    if (!glyph_index) {
